...
Lưu ý về mặt ký hiệu toán học: Trong khi Mục 4 giới thiệu phương pháp Double Machine Learning (DML) dựa trên mô hình tuyến tính từng phần (Partially Linear Model) để người đọc dễ tiếp cận thông qua trực quan của định lý Frisch-Waugh-Lovell, các ứng dụng thực tế dưới đây (ADE và GATE) sẽ mở rộng DML sang mô hình kết quả tiềm năng phi tham số tổng quát $f^*(D_t, X_t, H_t)$. Khi đó, tác động nhân quả không còn được biểu diễn qua một tham số tuyến tính duy nhất $\theta_0$ mà được ước lượng thông qua hiệu số kỳ vọng của các kết quả tiềm năng phi tuyến.
...
Bộ ước lượng được xây dựng dưới dạng trung bình mẫu của hàm score \(\phi\) \cite{hays2025double}:
\[
...
\begin{itemize}
    ...
    \item \textbf{Debiasing:} Dùng propensity score \(m(X_t)\) để đánh trọng số lại phần dư và sửa lỗi cho mô hình kết quả. Điểm khác biệt so với AIPW chuẩn \cite{robins1994estimation}: mô hình kết quả \(f\) \textbf{bắt buộc phải nhận \(H_t\) làm đầu vào} để kiểm soát confounding qua trạng thái chung.
\end{itemize}

Dưới Assumption 5.1 (regularity: propensity bị chặn, outcome bounded) và Assumption 5.2 (tốc độ hội tụ: \(\hat{m}, \hat{f}\) hội tụ với tích tốc độ \(o_P(T^{-1/2})\)), \textbf{Theorem 5.3 \cite{hays2025double}} kết luận:
\[
...
Hàm score cho GATE \cite{hays2025double}:
\begin{align}
...
Dưới Assumption 5.4 (xác suất chuỗi thuần nhất bị chặn: \(\pi^*((t{-}m):t, d) \geq \zeta > 0\)) và điều kiện tốc độ hội tụ tương tự, bộ ước lượng GATE cũng đạt chuẩn tiệm cận bình thường \cite{hays2025double}.
