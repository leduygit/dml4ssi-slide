...
\subsection{\'{Y} tưởng cốt lõi: Định lý Frisch-Waugh-Lovell (FWL)}
...
DML \cite{chernozhukov2018double} tổng quát hóa ý tưởng này: thay vì dùng hồi quy tuyến tính để làm sạch, sử dụng các mô hình học máy linh hoạt.
...
\begin{algorithmic}[1]
    ...
    \State \textbf{Bước 2:} Áp dụng \(\hat{l}, \hat{m}\) lên \(W_{1:T}\) để tính phần dư \(\tilde{Y}_t, \tilde{D}_t\)
...
\subsection{Định lý chính (Theorem 4.3)}

\subsubsection{Các giả định cần thiết}

Ngoài hai giả định về cấu trúc chuỗi Markov (Assumption 3.1a/b), định lý yêu cầu thêm hai điều kiện:

\paragraph{Assumption 4.1 --- Neyman Orthogonality (Trực giao Neyman):}
...
\paragraph{Assumption 4.2 --- Điều kiện hội tụ (Convergence rate conditions):}
...
\textbf{Theorem 4.3 \cite{hays2025double}:} Dưới Assumptions 3.1, 4.1 và 4.2, với xác suất không nhỏ hơn \(1 - \gamma\):
\[
...
Sau khi đã có Assumptions 4.1 và 4.2, bước còn lại của chứng minh là kiểm soát các hiệp phương sai chuỗi \(\operatorname{Cov}(\phi_t, \phi_s)\) với \(t \neq s\) --- điều mà DML i.i.d.\ thông thường không cần làm. Hai giả định Markov đóng vai trò khác nhau trong bước này:

\paragraph{Dưới Assumption 3.1(a) --- Geometric Ergodicity:}
...
Tuy nhiên, ước lượng phương sai \(\hat{\sigma}^2\) dưới điều kiện này \textbf{không thể dùng plug-in estimator} --- vì plug-in variance estimator cho chuỗi Markov không nhất quán nói chung \cite{jones2006fixed}. Bài báo đề xuất một bộ ước lượng phương sai nhất quán riêng không phải dạng plug-in.

\paragraph{Dưới Assumption 3.1(b) --- \(m\)-Dependence:}
