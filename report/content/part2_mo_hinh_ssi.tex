...
Khác với SUTVA (kết quả chỉ phụ thuộc \(D_t\)), \h{ơ} đây \(Y_t\) bị đồng quyết định bởi cả \(D_t\) lẫn trạng thái chung \(H_t\). Tồn tại vòng lặp phản hồi hai chiều: \(H_t \to Y_t\), và ngược lại \((D_t, X_t) \to H_{t+1}\). Cấu trúc phụ thuộc này được minh họa trong Hình~\ref{fig:dag-ssi}.
...
% H_t shared state
\node[shared, right=2.6cm of Dt1] (Ht) {$H_t$};
\node[font=\footnotesize\itshape, blue!80, below=0.1cm of Ht] {trạng thái chung};
...
\subsection{Các điều kiện của Chuỗi Markov (Assumption 3.1)}
